...
# ==========================================
    ...
    st.warning("""
    **🚨 核心警戒：嘔吐與胰囊壓迫**
    胰臟體部囊腫已達 **21.4mm x 21.8mm**。
    囊腫若持續擴大會壓迫十二指腸，導致胃排空受阻及頻繁噁心（舔嘴）。
    """)

    # 影像對照區塊
    st.subheader("🖼️ 2026/02/24 基準影像 (四月底追蹤對照用)")
    col_img1, col_img2 = st.columns(2)
    
    with col_img1:
        st.image("小豹 20260224 長青超音波胰體囊.jpg", 
                 caption="胰體部巨大囊腫 (21.42mm / 21.76mm)", 
                 use_container_width=True)
        st.info("💡 觀察點：囊腫壁厚度及內部液體回音。")

    with col_img2:
        st.image("小豹 20260224 長青超音波左胰囊.jpg", 
                 caption="左側胰臟囊腫 (10.24mm / 6.01mm)", 
                 use_container_width=True)
        st.info("💡 觀察點：左側胰臟實質是否有進一步炎症。")

    st.divider()

    # 嘔吐與餵食策略 (與影像邏輯結合)
    tab1, tab2 = st.tabs(["🤢 嘔吐與噁心管理", "💉 胰島素與餵食"])
    
    with tab1:
        st.markdown("""
        ### ⚠️ 嘔吐預警指標
        * **頻率監控**：若 24 小時內嘔吐超過 2 次，需立刻考慮囊腫造成急性阻塞。
        * **前驅徵兆**：頻繁舔嘴、流口水、拒絕進食、母雞蹲（腹痛）。
        * **藥物干預**：
            * 軟便劑應與主藥隔開 **2小時**（避免影響吸收）。
            * 便秘會增加腹壓，直接導致胰臟痛。
        """)
